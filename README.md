...
docker compose up -d
````
This starts **Job Manager**, **Task Managers**, **kafka brokers**, and **edge node**
...
Go inside the edge node container to manually run flink app
```text
docker compose exec edge bash
````
Inside the container, run : 
```text
flink run -m flink-jobmanager:8081 --pyFiles src.zip --pyRequirements requirements.txt --python src/pyflink_example/fraud_detection/main.py --config-path ./config/application.conf
````
...
Command to produce messages in the kafka topic 
```text
...
Command to consume messages in the kafka topic 
```text
